...
def cords_to_stickhm(kp_array, img_size, point_radius=1, sigma=4):
    ...
    mask = np.zeros(shape=img_size + (limbs.shape[0],), dtype=np.float32)
    idx = 0
    for f, t in limbs:
        ...
        mask[yy, xx, idx] = 2
        mask[:, :, idx] = gaussian_filter(mask[:, :, idx], sigma)
        idx += 1
...
if __name__ == "__main__":
    ...
    for index, row in df.iterrows():
        ...
        # result = cords_to_map(pose_cords, (128, 64))
        result = draw_pose_from_cords(pose_cords, (128,64))
        print(result[0].shape)
        # cv2.imshow('fdffd',result[1].astype(np.uint8)*255) #np.sum(result, axis=-1))
        # cv2.waitKey(1000)
        ...
        mmm = cords_to_stickhm(pose_cords, (128, 64))
        mmm2 = cords_to_map(pose_cords, (128, 64))
        print(mmm.shape)
        tmp = np.zeros(mmm.shape[0:2])
        # for i in range(0,23):
        #     tmp += mmm[:,:,i]
        cv2.imshow('fdfd',np.sum(mmm, axis=-1))
        cv2.imshow('fff',np.sum(mmm2, axis=-1))
        cv2.waitKey(1000)
        print(np.max(mmm2))
        # print(mmm.shape)
