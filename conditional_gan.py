...
from keras.layers import Flatten, Concatenate, Activation, Dropout, Dense, Add
    ...
    for i, (sk_app, sk_pose) in enumerate(zip(skips_app, skips_pose)):
        if i < 4:
            ...
            #     deformation 파트가 이부분
                ...
                out = sk_app
                out = Conv2D(int(out.shape[-1]), kernel_size=(3, 3), padding='same')(out)
                out = Conv2D(int(out.shape[-1]), kernel_size=(3, 3), padding='same')(out)
                # -------------------------------------------------------------------------
                out = Add()([out, sk_app])
...
def make_generator(image_size, use_input_pose, warp_skip, disc_type, warp_agg, use_bg, pose_rep_type):
    ...
    output_pose = Input(list(image_size) + [18 if pose_rep_type == 'hm' else 23 if pose_rep_type == 'stickhm' else 3])
    ...
    if use_input_pose:
        input_pose = [Input(list(image_size) + [18 if pose_rep_type == 'hm' else 23 if pose_rep_type == 'stickhm' else 3])]
    else:
        input_pose = []
