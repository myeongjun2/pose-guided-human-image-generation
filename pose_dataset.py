...
class PoseHMDataset(UGANDataset):
    ...
    def compute_pose_map_batch(self, pair_df, direction):
        ...
        batch = np.empty([self._batch_size] + list(self._image_size) + [18 if self._pose_rep_type == 'hm' else 23 if self._pose_rep_type == 'stickhm' else 3])
        i = 0
        for _, p in pair_df.iterrows():
            ...
            if self._cache_pose_rep:
                ...
                if os.path.exists(file_name):
                    ...
                    elif self._pose_rep_type == 'stickman':
                        pose = pose_transform.make_stickman(kp_array, self._image_size)
                    elif self._pose_rep_type == 'stickhm':
                        pose = pose_utils.cords_to_stickhm(kp_array, self._image_size)
    ...
    def display(self, output_batch, input_batch):
        ...
        if self._pose_rep_type == 'hm':
            ...
            pose_images = (255 * np.sum(tg_pose, axis=-1)).astype('uint8')
            pose_images = np.stack((pose_images,pose_images,pose_images), axis=-1)
